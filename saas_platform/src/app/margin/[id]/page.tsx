...
import { DownloadBundleButton } from "../../lab/l2/[id]/DownloadBundleButton";
...
export default async function ScanDetailPage({
  ...
  const bundleMd = renderMarginBundleMd(row.title ?? "scan", input, output);
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <DownloadBundleButton
            filename={`margin-${row.id.slice(0, 8)}.md`}
            content={bundleMd}
          />
          <DeleteSessionButton id={row.id} title={row.title ?? "scan"} />
        </div>
      </div>
...
function renderMarginBundleMd(
  title: string,
  input: MarginInput,
  output: ScanResult
): string {
  const filterLines = FILTERS.map((f) => {
    const result = output.filter_results[f.id as FilterId];
    const rationale = output.rationale[f.id as FilterId] ?? "";
    return `- [${result.toUpperCase()}] **${f.name}** — ${rationale}`;
  }).join("\n");

  return `# Margin Shield Scan — ${title}

**Verdict**: ${output.verdict}
**Margin**: ${output.computed.margin_pct}%
**Landed Cost**: $${output.computed.landed_cost_usd.toFixed(2)}
**HTS**: ${output.hts.matched ? `${output.hts.code} (${(output.hts.rate * 100).toFixed(1)}%)` : "FALLBACK 25%"}

---

## 입력
- 상품: ${input.productName}
- 소싱가: $${input.sourcingUsd}
- 배송비: $${input.shippingUsd}
- 무게: ${input.weightG}g
- 판매가: $${input.sellingUsd}
${input.url ? `- URL: ${input.url}` : ""}

## 7-Filter Kill Criteria

${filterLines}

---

> Generated by Command Center Margin Shield. 강의 연계: W2 Kill Criteria · W3 Landed Cost.
`;
}
