...
### 신고 면제 조건 (2026년 기준 — 공정거래위원회 고시)

다음 **둘 중 하나(OR)** 충족 시 통신판매업 신고가 **면제**됩니다 ([공정거래위 고시 — 통신판매업 신고면제기준](https://www.law.go.kr/행정규칙/통신판매업신고면제기준에대한고시)):

1. **거래 횟수가 직전년도(또는 최근 6개월) 50건 미만**
2. **간이과세자**

(즉, 둘 중 어느 하나라도 충족하면 면제 — AND가 아니라 OR)

> ⚠️ **2025-2026 추가 조건**: 일부 자료는 "최근 6개월 거래액 1,200만원 미만 + 20회 미만"을 추가 트랙으로 인용. 본인 케이스가 경계선이면 **시·군·구청 또는 정부24 사전 문의 필수** (잘못 미신고 시 과태료 200만원 이하). 2026.4 시점 정확한 조항은 본인이 정부24·홈택스에서 직접 확인.

> 둘 다 미충족(50건 이상 + 일반과세자) 시 시·군·구청에 통신판매업 신고 (수수료 약 4만 원, 1주일 소요).
...
START
  │
  ├─ Q1. 본인이 일반과세자(연 매출 8,000만 원+)인가?
  │     YES → ❶ 일반과세자 + Q2로 (간이 트랙 X)
  │     NO  → 간이과세자 → 면제 충족 (별도 신고 불요)
  │
  ├─ Q2. 직전년도(또는 최근 6개월) 거래 횟수가 50건 이상이었나?
  │     YES → ❷ 통신판매업 신고 필수
  │     NO  → 면제 (일반과세자라도 50건 미만이면 면제)
  │
...
### Dispute Rate 임계치 — 모니터링 권장 기준 (공식 hard rule 아님)

| 처리사 | 모니터링 권장 임계치 | 실제 정지 트리거 |
|---|:--:|---|
| Stripe | 0.75% (자체 권장) | 카드망 룰(VAMP·ECM)이 실제 모니터링 — Stripe는 reserve·hold 형태로 대응. fixed threshold 비공개 |
| PayPal | 1.0% | 1.5% + 100건 (HVDF, High Volume Dispute Fee) |
| Shopify Payments | 1.0% | 1.5% (Stripe 백엔드 기준) |
| **Visa VAMP** (카드망) | — | **1.5% + 100건/월** (실제 차단 기준) |
| **Mastercard ECM** | — | **1.5% + 100건/월** |

> ⚠️ **중요**: Stripe는 공식 fixed dispute threshold를 공개하지 않습니다. "0.75%"는 업계 권장 모니터링 기준이며, 실제 정지 결정은 **카드망 룰(Visa VAMP, Mastercard ECM)** + Stripe 자체 risk 모델 종합. **본인 케이스 정확한 임계값은 Stripe Support 또는 acquirer에 직접 문의**.
